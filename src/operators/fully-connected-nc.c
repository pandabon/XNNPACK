...
enum xnn_status xnn_create_fully_connected_nc_bf16(
  ...
  if (gemm_config == NULL) {
    xnn_log_error(
        "failed to create %s operator: unsupported hardware configuration",
        xnn_operator_type_to_string(
            xnn_operator_type_fully_connected_nc_bf16));
    return xnn_status_unsupported_hardware;
  }

  if (isnan(output_min)) {
    xnn_log_error(
        "failed to create %s operator with NaN output lower bound",
        xnn_operator_type_to_string(
            xnn_operator_type_fully_connected_nc_bf16));
    return xnn_status_invalid_parameter;
  }
  if (isnan(output_max)) {
    xnn_log_error(
        "failed to create %s operator with NaN output upper bound",
        xnn_operator_type_to_string(
            xnn_operator_type_fully_connected_nc_bf16));
    return xnn_status_invalid_parameter;
  }
  ...
  // bf16_minmax_params and f32_minmax_params have identical layout (float min, max).
  // Initialize as f32_minmax to avoid ABI mismatch with the bf16 init function
  // which expects xnn_bfloat16 struct args.
  struct xnn_f32_minmax_params params;
...
enum xnn_status xnn_reshape_fully_connected_nc_bf16(
    ...
    pthreadpool_t threadpool) {
  return reshape_fully_connected_nc(
      fully_connected_op, xnn_operator_type_fully_connected_nc_bf16,
      batch_size,
      /*log2_input_element_size=*/XNN_LOG2_SIZEOF_HALF,
      /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_HALF,
      /*filter_is_nibble=*/false,
...
enum xnn_status xnn_setup_fully_connected_nc_bf16(
    xnn_operator_t fully_connected_op, const void* input, void* output) {
  return setup_fully_connected_nc(
      fully_connected_op, xnn_operator_type_fully_connected_nc_bf16, input,
      output, /*workspace=*/NULL, /*quantization_params=*/NULL);
}
