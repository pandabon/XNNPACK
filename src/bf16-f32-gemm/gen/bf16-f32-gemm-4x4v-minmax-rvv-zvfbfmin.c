...
//   1 * vuint16m2_t  (vb_u16) =  2 vregs
//   1 * vfloat32m4_t (vb)     =  4 vregs
//   Total: 22 / 32
//
// Each Zvfbfmin asm block opens with its own vsetvli (e16, m2) so the
// convert is correct regardless of the surrounding e32m4 vtype.
    ...
    const struct xnn_f32_minmax_params* restrict params)
{
  ...
  do {
    ...
    size_t k = kc;
    do {
      uint32_t va0b = (uint32_t) (*a0k++) << 16; float va0; memcpy(&va0, &va0b, sizeof(float));
      uint32_t va1b = (uint32_t) (*a1k++) << 16; float va1; memcpy(&va1, &va1b, sizeof(float));
      uint32_t va2b = (uint32_t) (*a2k++) << 16; float va2; memcpy(&va2, &va2b, sizeof(float));
      uint32_t va3b = (uint32_t) (*a3k++) << 16; float va3; memcpy(&va3, &va3b, sizeof(float));
