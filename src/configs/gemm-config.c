...
// bf16->bf16 GEMM config: RVV Zvfbfmin path (Saturn GENV256D128 and similar).
static void init_bf16_gemm_config(void) {
  ...
  if (hardware_config->arch_flags & xnn_arch_riscv_vector) {
    ...
    // NR = vsetvlmax_e32m4() at runtime; approximated here as vlenb.
    // XNNPACK's operator setup will call the packw with nr=vlenb.
