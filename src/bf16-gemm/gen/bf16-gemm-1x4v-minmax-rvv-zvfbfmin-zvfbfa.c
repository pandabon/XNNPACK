...
// Combined Zvfbfmin+Zvfbfa path. Selected when both extensions are
// available; takes the best of each:
//   - Inner K-loop FMA: vfwmacc.vf under altfmt=1 (Zvfbfa). One
//     instruction widening BF16 x BF16 -> FP32, no separate widen of
//     B, 18/32 vreg budget at LMUL=m4.
//   - Epilogue narrow:  vfncvtbf16.f.f.w (Zvfbfmin). Plain e16,m2
//     vsetvli the toolchain assembler accepts directly, so the narrow
//     does not need the .insn i altfmt encoding.
//
// Toolchain note: the inner-loop altfmt vsetvli is still emitted as
// `.insn i 0x57, 0x7, rd, rs1, 0x1C9` because the assembler does not
// recognise e16alt. Immediate 0x1C9 is per Zvfbfa v0.1: bit 8=altfmt,
// bit 7=vma, bit 6=vta, bits[5:3]=vsew=001 (SEW=16), bits[2:0]=vlmul=
// 001 (m2). vfwmacc.vf uses the standard RVV 1.0 encoding.
...
void xnn_bf16_gemm_minmax_ukernel_1x4v__rvv_zvfbfmin_zvfbfa(
    size_t mr,
    size_t nc,
    size_t kc,
